...
local function GetRaidColor(name)
	if name:lower():find("blackwing Descent") or name:lower():find("bastion of twilight") or name:lower():find("throne of the four winds") 
		or name:lower():find("твердыня крыла тьмы") or name:lower():find("сумеречный бастион") or name:lower():find("трон четырех ветров") then
		return "|cFFFF7D0A" -- Orange
	elseif name:lower():find("firelands") or name:lower():find("огненные просторы") then
		return "|cFF00FF00" -- Green
	elseif name:lower():find("dragon soul") or name:lower():find("душа дракона") then
