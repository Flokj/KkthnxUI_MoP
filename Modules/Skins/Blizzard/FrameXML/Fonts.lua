...
local function ReskinFont(font, size)
	if not font then
		if K.isDeveloper then print("UNKNOWN FONT.") end
		return
	end
	local oldSize = select(2, font:GetFont())
	size = size or oldSize
	local fontSize = size*C["Skins"].FontScale
	K.SetFontSize(font, size*C["Skins"].FontScale)
...
tinsert(C.defaultThemes, function()
	...
	if not C["Skins"].FontOutline then return end
