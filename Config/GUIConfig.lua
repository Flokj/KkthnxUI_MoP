...
local function CreateAuraWatchCategory()
	...
	GUI:CreateSlider(generalAuraWatchSection, "AuraWatch.MinCD", L["AuraWatch MinCD"], 1, 60, 1, L["MinCDTip"], UpdateAuraWatchSettings)
...
local function CreateAutomationCategory()
	...
	GUI:CreateSwitch(inviteSection, "Automation.AutoLoggingCombat", "Auto enables Combat Log in raid instances", updateAutoInvite)
	...
	local function updateAutomationMisc(newValue, oldValue, configPath)
		if K.GetModule then
			...
			if automationModule then
				if configPath:find("AutoOpenItems") and automationModule.UpdateAutoOpenItems then
...
local function CreateInventoryCategory()
	...
	GUI:CreateSwitch(bagBarSection, "Inventory.BagBarMouseover", "Show Bag Bar On Mouseover", UpdateInventorySettings)
...
local function CreateMiscCategory()
	...
	GUI:CreateSwitch(generalMiscSection, "Misc.AlreadyKnown", "Highlight Already Known Items", "Highlights items you already know in various frames such as the merchant, auction house, and guild bank.", UpdateMiscSettings)
	GUI:CreateSwitch(generalMiscSection, "Misc.ClassColorPlus", "Enable Class Color Enhancements", "Enhances various UI elements with class-specific colors, including guild, friends, who list, and battlefield score frames.", UpdateMiscSettings)
	...
	GUI:CreateSwitch(generalMiscSection, "Misc.Focuser", "Mouseover focus by Alt + LeftClick", UpdateMiscSettings)
	...
	GUI:CreateSwitch(generalMiscSection, "Misc.SlotDurabilityWarning", "Show Low Durability Warnings.", "Enable this option to display a warning when your equipment's durability falls below 25%. The warning will remind you to repair your gear to avoid item breakage.", UpdateMiscSettings)

	-- ========================================
	-- CAMERA SECTION
	-- ========================================
	local cameraSection = GUI:AddSection(miscCategory, "Camera")
	GUI:CreateSlider(cameraSection, "Misc.MaxCameraZoom", "Max Camera Zoom Level", 1, 3.4, 0.1, "Set the maximum camera zoom distance", UpdateMaxZoomLevel)
	...
	GUI:CreateSwitch(mailSection, "Misc.MailSaver", "Mail Saver (Save mail recipient)", UpdateMiscSettings)
	...
	GUI:CreateSwitch(questingSection, "Misc.ShowWowHeadLinks", L["Show Wowhead Links Above Questlog Frame"], "Displays helpful Wowhead links above the quest log frame", UpdateMiscSettings)
	...
	GUI:CreateSwitch(threatBarSection, "Misc.ThreatEnable", "Show Threat Bar", UpdateMiscSettings)
	GUI:CreateSwitch(threatBarSection, "Misc.ThreatHideSolo", "Show only in party/raid", UpdateMiscSettings)
	GUI:CreateSlider(threatBarSection, "Misc.ThreatHeight", "Bars height", 1, 40, 1, nil, UpdateMiscSettings)
	GUI:CreateSlider(threatBarSection, "Misc.ThreatWidth", "Bars width", 10, 400, 1, nil, UpdateMiscSettings)
	GUI:CreateSlider(threatBarSection, "Misc.ThreatBarRows", "Number of bars", 1, 12, 1, nil, UpdateMiscSettings)
	...
	GUI:CreateSwitch(miscellaneousSection, "Misc.HelmCloakToggle", "Add Helm/Cloak Toggle Buttons To Character Frame", UpdateMiscSettings)
end
...
local function CreateNameplateCategory()
	...
	GUI:CreateSwitch(miscellaneousNameplateSection, "Nameplate.TarName", "Show target name", refreshNameplates)
...
local function CreateSkinsCategory()
	...
	GUI:CreateSwitch(blizzardSkinsSection, "Skins.TradeSkills", "Skin TradeSkills", UpdateSkinsSettings)
	GUI:CreateSwitch(blizzardSkinsSection, "Skins.Trainers", "Skin Trainers", UpdateSkinsSettings)
	...
	GUI:CreateSwitch(addonSkinsSection, "Skins.AtlasLoot", "AtlasLoot Skin", UpdateSkinsSettings)
	...
	GUI:CreateSwitch(addonSkinsSection, "Skins.Skada", L["Skada Skin"], "Apply KkthnxUI styling to Skada! damage meter", UpdateSkinsSettings)
...
local function CreateTooltipCategory()
	...
	-- Hide in Combat Options
	local hideInCombatOptions = {
		{ text = DISABLE, value = 1 },
		{ text = "ALT", value = 2 },
		{ text = "SHIFT", value = 3 },
		{ text = "CTRL", value = 4 },
	...
	GUI:CreateDropdown(generalTooltipSection,"Tooltip.HideInCombat", L["Hide Tooltip in Combat"], hideInCombatOptions, "Hide tooltips during combat to reduce screen clutter", UpdateTooltipSettings)

	-- ========================================
	-- APPEARANCE SECTION
	-- ========================================
	local appearanceTooltipSection = GUI:AddSection(tooltipCategory, "Appearance")
	GUI:CreateSwitch(appearanceTooltipSection, "Tooltip.ItemQuality", L["Quality Color Border"], "Color tooltip borders based on item quality or unit class", UpdateTooltipSettings)
...
local function CreateUnitframeCategory()
	...
	GUI:CreateSlider(generalUnitframeSection, "Unitframe.AllTextScale", "Scale All Unitframe Texts", 0.8, 1.5, 0.05, "Experimental: Scale all text on unitframes", UpdateUFTextScale)

	-- CLASS RESOURCE SECTION
	local classResourceSection = GUI:AddSection(unitframeCategory, "Class Resources")
	GUI:CreateSwitch(classResourceSection, "Unitframe.ClassResources", L["Show Class Resources"], "Display class-specific resource bars (combo points, soul shards, etc.)", UpdateUnitframeSettings)
	GUI:CreateSwitch(classResourceSection, "Unitframe.ResourceColorMode", "Use the primary color regardless of the bar's fullness", UpdateUnitframeSettings)
	...
	GUI:CreateSlider(targetUnitframeSection, "Unitframe.TargetBuffsNum", "Max Buffs on Target", 3, 60, 1, "Max count of buff icons on target frame", UpdateTargetBuffs)
	GUI:CreateSlider(targetUnitframeSection, "Unitframe.TargetDebuffsNum", "Max Debuffs on Target", 3, 60, 1, "Max count of debuff icons on target frame", UpdateTargetDebuffs)
...
local function CreateWorldMapCategory()
	...
	GUI:CreateSlider(sizesWorldMapSection, "WorldMap.MaxMapScale", "Fullscreen map scale", 0.1, 1, 0.1, nil, UpdateMapSize)
	GUI:CreateSlider(sizesWorldMapSection, "WorldMap.MapScale", "Window map scale", 0.5, 2, 0.1, nil, UpdateMapSize)
end
